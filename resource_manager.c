...
/*   Updated: 2025/02/12 17:47:08 by pabalons         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void free_images(t_map *map)
{
    ...
    if (map->img.player_left1) { mlx_delete_image(map->mlx, map->img.player_left1); map->img.player_left1 = NULL; }
    if (map->img.player_right1) { mlx_delete_image(map->mlx, map->img.player_right1); map->img.player_right1 = NULL; }
    if (map->img.player_up1) { mlx_delete_image(map->mlx, map->img.player_up1); map->img.player_up1 = NULL; }
    if (map->img.player_down1) { mlx_delete_image(map->mlx, map->img.player_down1); map->img.player_down1 = NULL; }
    if (map->img.player_left2) { mlx_delete_image(map->mlx, map->img.player_left2); map->img.player_left2 = NULL; }
    if (map->img.player_right2) { mlx_delete_image(map->mlx, map->img.player_right2); map->img.player_right2 = NULL; }
    if (map->img.player_up2) { mlx_delete_image(map->mlx, map->img.player_up2); map->img.player_up2 = NULL; }
    if (map->img.player_down2) { mlx_delete_image(map->mlx, map->img.player_down2); map->img.player_down2 = NULL; }
    if (map->img.player_down_basic1) { mlx_delete_image(map->mlx, map->img.player_down_basic1); map->img.player_down_basic1 = NULL; }
    
    if (map->img.empty) { mlx_delete_image(map->mlx, map->img.empty); map->img.empty = NULL; }
    if (map->img.wall) { mlx_delete_image(map->mlx, map->img.wall); map->img.wall = NULL; }
    if (map->img.exit) { mlx_delete_image(map->mlx, map->img.exit); map->img.exit = NULL; }
    if (map->img.exit_open) { mlx_delete_image(map->mlx, map->img.exit_open); map->img.exit_open = NULL; }
    if (map->img.collectible) { mlx_delete_image(map->mlx, map->img.collectible); map->img.collectible = NULL; }
}
